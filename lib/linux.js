...
const parseLinux = ({stdout, boundsStdout, activeWindowId}) => {
	...
	return {
		platform: 'linux',
		title: (function() {
		            let name = result['_NET_WM_NAME(UTF8_STRING)'] || result['WM_NAME(STRING)'];
		            if (name) {
		                // Check if name contains '\\342\\200\\224' and replace it with '-'
		                if (name.includes('\\342\\200\\224')) {
		                    name = name.replace('\\342\\200\\224', '-');
		                }
		            }
		            return name || null;
		        })(),
